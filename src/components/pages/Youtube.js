...
function YouVideo() {
  ...
  const search = (query) => {
    ...
    fetch(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=${query}&type=video&key=AIzaSyD5lLAYSLZ2XrL6ttq4S8sFmWJ0cyFOFtg`,
      requestOptions
    )
  ...
  useEffect(() => {
    ...
    fetch(
      "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=javascript&type=video&key=AIzaSyD5lLAYSLZ2XrL6ttq4S8sFmWJ0cyFOFtg",
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => setVideos(result.items))
